...
fn part2(input: &str) -> usize {
    ...
    let seeds_str = lines.drain(..1).collect::<String>();

    let old_seeds = seeds_str.split_once(':').unwrap().1
        .split(' ')
        .filter(|s| !s.is_empty())
        .map(|s| s.parse::<usize>().unwrap())
        .collect::<Vec<usize>>();

    let chunked: Vec<(usize, usize)> = old_seeds.chunks(2)
        .map(|a| (a[0], a[1]))
        .collect();

    let seeds: Vec<usize> = chunked.iter()
        .map(|a| (a.0..(a.0+a.1)).collect::<Vec<usize>>())
        .flatten()
        .collect();
